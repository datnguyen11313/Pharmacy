...
public class login extends JFrame {
	...
	public login() {
		...
		var loginform = new JPanel();
		loginform.setBounds(0, 0, 322, 460);
		contentPane.add(loginform);
		loginform.setLayout(null);

		var loginUsername = new JLabel("Username :");
		...
		var lblNewLabel = new JLabel("PharmaStore");
		...
		btnLogin.addActionListener(e -> {
			...
			if (conn != null) {
				...
				try (var stmt = conn.prepareStatement(query)) {
					...
					stmt.setString(2, password1); // Thêm mật khẩu vào câu truy vấn

					var rs = stmt.executeQuery();

					if (rs.next()) {
						// Đăng nhập thành công
						var role = rs.getString("role_id"); // Lấy role_id từ cơ sở dữ liệu
						JOptionPane.showMessageDialog(null, "Login Successful");

						// Chuyển đến Main với role tương ứng
						var mainFrame = new Main(role); // Truyền role vào Main
